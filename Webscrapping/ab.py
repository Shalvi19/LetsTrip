...
def requesthandle( link, name ):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            f.close()
            print ("[*] Downloaded Image: {}" ,format(name))
    except Exception as error:
        print ("[~] Error Occured with {}: {}",format(name, error))
    THREAD_COUNTER -= 1

def main():
    html = get_source( "https://www.treebo.com/blog/fun-things-to-do-in-jaipur/" )
    tags = filter( html )
    for tag in tags:
        src = tag.get( "src" )
        if src:
            ...
            if src:
                ...
                if not link.startswith("http"):
                    link = "https://www.treebo.com/blog/fun-things-to-do-in-jaipur/" + link
